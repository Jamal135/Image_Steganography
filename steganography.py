...
def binary_encode(file: IO[bytes]):
    ''' Returns: File at data of data convert to binary. '''
    data_bytes = file.name + file.read()
...
def generate_message(Config: object, data: IO[bytes], coords: list):
...
def data_insert(image_file: IO[bytes], input_file: IO[bytes], key: str = '999', method: str = 'random',
    ...
    Config = build_object(image_key, method, stored,
                          encrypt, colours, indexs, noise)
...
def data_extract(file: IO[bytes], key: str = '999', overwrite: bool = True):
